...
class MCPHTTPClientApp(MCPHTTPClient):
    """Gradio GUI client for direct MCP HTTP server interaction."""
    ...
    def create_interface(self):
        with gr.Blocks(title="MCP HTTP Client") as interface:
            ...
            gr.Markdown(f"**Server:** {self.server_url} | **Workspace:** {self.roots_dir}")
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python3 http_client_app.py <server_url> <roots_dir>")
        print("Example: python3 http_client_app.py http://127.0.0.1:8000 ./workspace")
        sys.exit(1)
