...
def render_pretty_table_html(df: pd.DataFrame, title: str = "Data Table") -> str:
    ...
    .styled-table {{
        ...
        width: 100%; color: black; box-shadow: 0 0 5px rgba(0,0,0,0.1);
    }}
    .styled-table th, .styled-table td {{ border: 1px solid #ddd; padding: 8px; }}
    .styled-table th {{ background-color: #007acc; color: white; text-align: left; }}
...
def render_image_with_quote_html(image_url: str, quote: str, width: int = 512) -> None:
    ...
    <div style="position:relative;width:{width}px;margin-bottom:20px;">
        ...
        <div style="position:absolute;bottom:20px;left:50%;transform:translateX(-50%);
                    ...
                    font-size:1.2em;font-family:'Segoe UI',sans-serif;font-weight:500;
                    text-align:center;text-shadow:1px 1px 4px #000;">
            {escape(quote)}
        </div>
    </div>
    """
...
def log_tool_call_html(tool_name: str, arguments: Any) -> None:
    ...
    <div style="border-left:4px solid #1976D2;padding:.8em;margin:1em 0;
        ...
        <div style="font-size:15px;font-weight:bold;margin-bottom:4px;">
            <span style="color:#0B3D91;">Tool Call:</span>
            <span style="color:#0B3D91;">{escape(str(tool_name))}</span>
        </div>
        <code style="display:block;background:#e8f0fe;color:#1b1b1b;padding:6px;
                     border-radius:4px;font-size:13px;white-space:pre-wrap;">{escape(str(arguments))}</code>
    </div>
    """))
...
def log_unexpected_html() -> None:
    ...
    <div style="border-left:4px solid #F57C00;padding:1em;margin:1em 0;
        ...
        <strong>Unexpected:</strong> No tool_calls or content returned.
    </div>
    """))
